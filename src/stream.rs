...
impl<S: Serialize + for<'a> Deserialize<'a> + PacketTrait + std::marker::Send + 'static> Stream<S> {
    ...
    pub async fn close(&self) -> Result<(), NetError> {
        ...
        trace!("Locked `is_stream_alive`.");
        if !*is_stream_alive_rlock {
            info!("Stream already closed.");
            return Ok(());
        };
        trace!("Stream geeting closed.");
        ...
        let result = match self.write(&packet.to_bytes()?).await.err() {
            Some(e) => match e {
                ...
                _ => {
                    debug!(
                        "Got error while closing stream to {}",
                        self.get_peer_ip().await
                    );
                    Err(e)
                }
            },
    ...
    /// Check if the receiver public key exists.
    /// When the receiver public key exists the all messages will be encrypted. // TODO: (ENCRYPTION COMING SOON)
    pub async fn is_encrypted(&self) -> bool {
        self.receiver_public_key.is_some()
    }
